...
public partial class Program
{
    public static async Task Main(string[] args)
    {
        ...
        {
            // Log to stderr to avoid interfering with STDIO JSON-RPC on stdout
            Console.Error.WriteLine($"❌ Fatal error starting MCP.Schema Server: {ex.Message}");
    ...
    public static IHostBuilder CreateHostBuilder(string[] args)
    {
        ...
        // This ensures appsettings.json is found regardless of the current working directory
        var executablePath = System.Reflection.Assembly.GetExecutingAssembly().Location;
        var executableDir = Path.GetDirectoryName(executablePath) ?? Directory.GetCurrentDirectory();

        return Host.CreateDefaultBuilder(args)
            .UseContentRoot(executableDir)
            ...
            .ConfigureAppConfiguration((context, config) =>
            {
                ...
                config.SetBasePath(executableDir)
            ...
            .ConfigureServices((context, services) =>
            {
                ...
                // Add Schema Manager HTTP client service (Singleton because MCP providers are Singleton)
                services.AddSingleton<ISchemaManagerClient, SchemaManagerClient>();
