...
public class SchemaPromptProvider : IMcpPromptProvider
{
    ...
    private async Task<List<McpPromptMessage>> GenerateAnalyzeSchemaPrompt(Dictionary<string, object>? arguments, CancellationToken cancellationToken)
    {
        ...
        var userMessage = new McpPromptMessage
        {
            ...
            Content = new McpPromptContent
            {
                Type = "text",
                Text = schema != null
                    ? $"You are a schema analysis expert. Analyze the provided schema definition for structure, patterns, potential issues, and best practices.\n\nPlease analyze this schema:\n\nName: {schema.Name}\nVersion: {schema.Version}\nType: {schema.SchemaType}\n\nDefinition:\n{schema.Definition}"
                    : "You are a schema analysis expert. Please analyze the schema definition provided in the arguments."
            }
        };

        return new List<McpPromptMessage> { userMessage };
    }
    ...
    private async Task<List<McpPromptMessage>> GenerateValidationGuidePrompt(Dictionary<string, object>? arguments, CancellationToken cancellationToken)
    {
        ...
        var userText = $"You are a data validation expert. Provide guidance on validating data against the provided schema.\n\nProvide validation guidance for this schema:\n\nName: {schema?.Name}\nVersion: {schema?.Version}\nDefinition:\n{schema?.Definition}";
