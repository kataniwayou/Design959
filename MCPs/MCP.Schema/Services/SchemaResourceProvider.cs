...
public class SchemaResourceProvider : IMcpResourceProvider
{
    ...
    public async Task<ReadResourceResponse> ReadResourceAsync(ReadResourceRequest request, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            // Parse URI manually to preserve case sensitivity
            // URI.Host automatically lowercases the host part, which breaks our case-sensitive composite keys
            if (!request.Uri.StartsWith("schema://"))
            {
                throw new ArgumentException($"Unsupported URI scheme. Expected 'schema://'");
            }

            var uriWithoutScheme = request.Uri.Substring("schema://".Length);
            var contents = new List<McpResourceContent>();

            if (uriWithoutScheme.StartsWith("collection/all"))
            {
                // Return all schemas
                var schemas = await _schemaClient.GetAllSchemasAsync(cancellationToken);
            ...
            else if (uriWithoutScheme.StartsWith("collection/version/"))
            {
                // Return schemas for specific version
                var version = uriWithoutScheme.Substring("collection/version/".Length);
            ...
            else
            {
                ...
                // Use the raw string to preserve case sensitivity
                var compositeKey = uriWithoutScheme;
